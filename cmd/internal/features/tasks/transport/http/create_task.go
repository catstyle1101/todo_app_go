...
type CreateTaskRequest struct {
	Title        string  `json:"title"          validate:"required,min=3,max=100"   example:"Домашнее задание"`
	Description  *string `json:"description"    validate:"omitempty,min=1,max=1000" example:"Сделать домашнее задание до 19:00"`
...
func (h *TaskHTTPHandler) CreateTask(rw http.ResponseWriter, r *http.Request) {
	...
	task, err := h.taskService.CreateTask(ctx, taskDomain)
	if err != nil {
		responseHandler.ErrorResponse(
			err,
